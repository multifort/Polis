...
def _patch(
    ...
    async def _rank(_s: Any, _org: Any, _v: list[float], limit: int = 10) -> list[PlanTemplate]:
        return ranked

    async def _list(_s: Any, _org: Any) -> list[PlanTemplate]:
        return listed
...
def test_semantic_picks_first_feasible_in_rank(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    chosen, sim = asyncio.run(service._retrieve_template(object(), object(), {"x"}, [0.0, 1.0]))
    assert chosen is b
...
def test_semantic_skips_infeasible_top(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    chosen, _ = asyncio.run(service._retrieve_template(object(), object(), {"x"}, [0.0, 1.0]))
...
def test_fallback_no_vec_first_feasible(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    chosen, sim = asyncio.run(service._retrieve_template(object(), object(), {"x"}, None))
...
def test_none_when_no_feasible(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    chosen, sim = asyncio.run(service._retrieve_template(object(), object(), {"x"}, [1.0, 0.0]))
