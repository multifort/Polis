...
>
> 技术上：给定意图，系统开出一家「虚拟公司(Org)」——内有按**角色(Role)**组织、可版本化的 **Agent**，
> 由 **Planner** 规划、按**能力路由**分派、在受治理（审批 / 护栏 / 凭证隔离）的运行时里协同完成目标。

**命名与定位**：品牌字标保留 `Polis`，对用户用中文定位「**虚拟智能企业平台**」让人一眼懂其作用（见 [ADR-0003](docs/decisions/0003-project-name-polis.md)）。
`Polis`（城邦）的隐喻仍是内核：每个 Org 即一个自治「城邦」——有公民(Agent)、角色分工、治理（审批/护栏）。
