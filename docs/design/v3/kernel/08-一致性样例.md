...
12 章闭合后的最小完整 Definition fixture 是
[`fixtures/generic-definition-set-v1.json`](fixtures/generic-definition-set-v1.json)。本章 Mermaid、转换表和
局部 JSON 继续用于解释场景，不再承担完整 Pydantic 输入合同；实现测试必须先解析该完整 fixture。
