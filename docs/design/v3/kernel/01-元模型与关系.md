...
及后续 `backend/tests/fixtures/kernel` 为准。
...
| `kernel_contract_version` | 本规格运行协议版本，当前 `3.3` |
...
4. 按 11 章校验所有 SchemaProfileV1、ConditionExprV1、MappingExprV1、状态、策略、触发器和能力 key；
...
`protocol-manifest.yaml` 3.3 中全部固定 Definition 与 Scope command discriminator，`data_scopes` 固定为
