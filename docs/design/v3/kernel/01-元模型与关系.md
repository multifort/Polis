...
`authority_constraints` 固定为 `AuthorityConstraintsV1`，不得接受其他字段：

```json
{
  "commands": ["update_scope"],
  "tools": [],
  "data_scopes": ["kernel.governance"],
  "max_risk_level": "high",
  "budget_cents": 100000
}
```

五个字段均可省略；省略表示该维度不再收窄，显式空数组表示该维度拒绝全部。出现时，
`commands/tools/data_scopes` 必须分别是 RoleDefinition 对应数组的集合子集，
`max_risk_level` 按 `low < medium < high < critical` 不得高于角色值，`budget_cents` 不得高于角色值。
数组必须无重复并按 canonical key 排序后持久化。任一维度扩大权限固定返回
`ASSIGNMENT_AUTHORITY_ESCALATION`；未知字段或结构错误返回 `ASSIGNMENT_AUTHORITY_INVALID`。
