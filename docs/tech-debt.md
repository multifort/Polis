...
| [TD-030](#td-030) | 模板/预设/记忆语义检索已落；能力语义=去重原语已落，**剩**执行期外的提案接线(随 TD-032) | Med | **大部分偿还** | 能力提案落地(TD-032)时接线 |
...
- **大部分已偿还（2026-06-27）**：模板语义=A1；预设语义=TD-017；org 记忆语义接地=B2。**能力语义去重原语**（§14.4）已落：`repo.rank_capabilities_by_vector` + `skillgen.resolve_capability`（embed→最近已有 key，≥τ_dedup(0.86)复用，否则新）+ 单测 `test_capability_dedup.py`。**剩**：其活跃调用方=能力提案（compose/A2 提新能力时去重），随 TD-032 接线。**注**：能力 key 是契约，**执行期路由保持精确匹配**，语义只用于登记去重（避免误路由）。技能/角色检索同理待 TD-032 提案链。
