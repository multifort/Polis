...
- MCP 已从纯本地工具推进到 HTTP tool bridge + MCP SDK transport adapter：
  ...
  stdio transport 已 fail-closed 接 `POLIS_MCP_STDIO_ALLOWED_COMMANDS` 命令白名单，运行时与 tool Skill
  草稿沙箱均会拒绝未放行 command；回归覆盖：
  `tests/test_mcp_runtime.py::test_runtime_rejects_unlisted_mcp_stdio_command`、
  `tests/test_skillgen_tool_sandbox.py::test_mcp_stdio_config_requires_allowed_command`。
  工具发现/同步底座已补：`discover_mcp_tools` 通过 MCP SDK `list_tools` 拉取工具 schema，
...
- **已偿还（2026-07-07）**：新增后端 `GET /api/skills` / `POST /api/skills`。
  ...
  `createToolSkill`，技能库页已支持提交 HTTP bridge tool 草稿。回归覆盖 tool 草稿沙箱持久化与
  越权权限拒绝。
  前端新增 `/orgs/{id}/skills` 技能库页与左侧导航入口，可列出私有/可见 Skill、筛选状态、提交
  manual 草稿并跳转审批收件箱。
- **后续增强**：published Skill 的新版/废弃/升级提示策略按需开放。
