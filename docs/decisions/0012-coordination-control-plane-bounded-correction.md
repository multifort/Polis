...
- **S4** ✅ 长 activity heartbeat（`run_node` 后台周期 heartbeat + `heartbeat_timeout` → worker 崩溃快速重试）；human gate durable wait（V1 已有）；Temporal 持久（天然）。
  **workflow versioning 已落首个 patch**（2026-07-10）：`quality-detail-v1` 保护新增质量明细结果；无 patch marker 的旧 history 走旧返回结构，新执行记录 marker 后返回 `quality_detail`。回归测试先由同名旧 Workflow 生成真实 history，再用当前 `TaskWorkflow` 的 `Replayer` 回放通过，后续编排行为变化继续使用稳定 patch ID。
